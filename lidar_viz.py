...
landcover_dict = {1: 'Trees', 2: 'Mostly Grass',
                  5: 'Road', 7: 'Building', 11: 'Cloth Panel'}
...
indx, indy = np.where((truth == 1) | (truth == 2) | (
    truth == 5) | (truth == 7) | (truth == 11))
...
for i in a:
    ...
    ax[count].hist(class_height, bins=120, color=colors[count],
                   label=landcover_dict[i], alpha=0.6)
...
for i in a:
    ...
    ax[count].hist(class_intensity, bins=150, color=colors[count],
                   label=landcover_dict[i], alpha=0.6)
...
fig.legend()
fig.suptitle(
    "Histogram of Intensity of different landcover classes", fontsize=36)
